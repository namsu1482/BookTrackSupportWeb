...
# 5. 모서리가 둥근 마스크 이미지 생성 함수 (이미지 코너 깎기용)
...
def generate_screenshots():
    # 캔버스 크기 (가로 1080 * 세로 1920)
    canvas_w, canvas_h = 1080, 1920
    
    # 디바이스 프레임 크기 및 위치 정의 (하단부 위주 배치)
    device_w, device_h = 700, 1400
    device_x = (canvas_w - device_w) // 2  # 가로 중앙 정렬: X=190
    device_y = 480                         # 상단 텍스트 영역 밑으로 배치: Y=480 ~ 1880
    
    # 내부 스크린 영역 크기 (베젤 두께 18px 차감)
    bezel = 18
    screen_w = device_w - (bezel * 2)  # 664px
    screen_h = device_h - (bezel * 2)  # 1364px
    
    # 부드러운 나눔스퀘어라운드 폰트 로드 (크기 키움)
    try:
        # 타이틀 크기 기존 56px -> 68px로 상향
        title_font = ImageFont.truetype(FONT_PATH_B, size=68)
        # 서브타이틀 크기 기존 30px -> 36px로 상향
        subtitle_font = ImageFont.truetype(FONT_PATH_R, size=36)
        print("나눔스퀘어라운드 폰트가 성공적으로 로드되었습니다.")
    except IOError:
        print("나눔스퀘어라운드 폰트 로드 실패. macOS 기본 폰트를 사용합니다.")
        try:
            title_font = ImageFont.truetype("/System/Library/Fonts/AppleSDGothicNeo.ttc", size=68, index=0)
            subtitle_font = ImageFont.truetype("/System/Library/Fonts/AppleSDGothicNeo.ttc", size=36, index=0)
        except IOError:
            title_font = ImageFont.load_default()
            subtitle_font = ImageFont.load_default()

    for config in SCREENSHOT_CONFIGS:
        # 파일명을 안전하게 매핑 (NFC, NFD 파일명 정규화 보완)
        target_filename = config.get("filename_nfc", config["filename"])
        screenshot_path = os.path.join(BASE_DIR, target_filename)
        
        if not os.path.exists(screenshot_path):
            import unicodedata
            normalized_name = unicodedata.normalize('NFC', target_filename)
            screenshot_path = os.path.join(BASE_DIR, normalized_name)
            if not os.path.exists(screenshot_path):
                normalized_name = unicodedata.normalize('NFD', target_filename)
                screenshot_path = os.path.join(BASE_DIR, normalized_name)

        # 예외 상황용 대비 코드 (NFC/NFD 모두 실패 시 기본 딕셔너리 명칭 탐색)
        if not os.path.exists(screenshot_path):
            screenshot_path = os.path.join(BASE_DIR, config["filename"])
            if not os.path.exists(screenshot_path):
        ...
        # 서브타이틀 줄바꿈 처리 및 가로 중앙 정렬 드로잉 (Y=210px 시작)
        ...
        # 스크린 모서리를 둥글게 깎기 위한 마스크 제작 (반경 35px)
        ...
        # 6. 현대식 다이내믹 아일랜드(카메라 홀 알약 모양) 드로잉
...
def generate_feature_graphic():
    ...
    # 2. 3단 레이아웃에 필요한 폰트 개별 로드 (스토어 스크린샷 폰트와 일관되게 통일)
    try:
    ...
    # 3. 좌측 3단 텍스트 구조 스타일 렌더링 (균형감 있는 배치를 위해 Y 좌표 보정)
    text_x = 90
    tag_y = 120
    title_y = 165
    slogan_y = 250
    
    # 1단: 카테고리 태그 (포인트 퍼플 컬러 #C084FC)
    tag_text = "D I G I T A L   B O O K   L I F E"
    draw.text((text_x, tag_y), tag_text, fill="#C084FC", font=tag_font)
    
    # 2단: 메인 슬로건 (스토어 스크린샷과 통일된 나눔스퀘어라운드 Bold 타이틀, 흰색)
    ...
    # 3단: 서브 설명 (체계적인 가독성을 위해 줄바꿈 처리 및 섀도 드로잉)
    ...
    # 4. 우측 목업 디바이스 렌더링 (인트로.jpg.jpeg 합성 - 하단 컷오프)
    intro_filename = "인트로.jpg.jpeg"
    intro_path = os.path.join(BASE_DIR, intro_filename)
    
    if not os.path.exists(intro_path):
        import unicodedata
        normalized_name = unicodedata.normalize('NFC', intro_filename)
        intro_path = os.path.join(BASE_DIR, normalized_name)
        if not os.path.exists(intro_path):
            normalized_name = unicodedata.normalize('NFD', intro_filename)
            intro_path = os.path.join(BASE_DIR, normalized_name)
    ...
    # 5. 인트로 스크린샷 크롭 및 내부 화면 매핑
    bezel = 10
    screen_w = device_w - (bezel * 2)  # 240px
    screen_h = device_h - (bezel * 2)  # 500px
    
    src_img = Image.open(intro_path).convert("RGBA")
    
    # 비율 유지 크롭 함수를 호출하여 인트로 화면 왜곡 방지 및 하단 잘림 해결
